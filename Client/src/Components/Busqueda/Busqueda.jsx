...
export default function Busqueda() {
    const {AgregarAlCarrito,favorite,DetalleProduct,putFavorite}= useProductsStore()
    ...
    const pedirResultados=async()=>{
        ...
        await fetch("http://localhost:8080/buscarproductos?"+'busqueda='+busquedaparams, options)
          .then((response) => response.json())
          .then((response) => {setResultado(response.busqueda)})
  ...
  return (
    <div>
      ...
      {Resultado&&Resultado.map((product,index)=>{
        return(
